...
def brut_force(points):
    minvalue = 1000000000000
    x = 0
    y = 1
    for i in range(len(points) - 1):
        for j in range(i + 1, len(points)):
            value = math.sqrt((points[i][x] - points[j][x]) ** 2 + (points[i][y] - points[j][y]) ** 2)
            if value < minvalue:
                minvalue = value
    return minvalue
...
if __name__ == "__main__":
    ...
    for k in range(1, 1000, 10):
        ...
        for i in range(1, k * 10, 1):
            points.append((random.randint(1,10000), random.randint(1,10000)))
        start_time = time.time()
        #brut_force(points)
        closest_pair_of_points(points, len(points))
        end_time = time.time()
        data.append({'x': len(points), 'y':round(end_time - start_time, 7)})